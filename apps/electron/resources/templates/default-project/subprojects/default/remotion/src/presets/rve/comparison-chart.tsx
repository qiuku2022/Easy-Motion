...
export function RveComparisonChart(props: RveBaseProps = {}) {
  const title = pText(props, "title", "Performance Comparison");
  const value = pNum(props, "value", 34);
  const progress = pNum(props, "progress", 89);
  ...
  const beforeValue = Math.round(
    interpolate(frame, [10, 40], [0, value], {
  ...
  const afterValue = Math.round(
    interpolate(frame, [20, 50], [0, progress], {
      extrapolateRight: "clamp",
      extrapolateLeft: "clamp",
    })
  );
