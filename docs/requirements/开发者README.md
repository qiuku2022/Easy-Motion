...
- **自然语言驱动**：用中文/英文描述需求，LangChain Agent 自动修改时间线、素材/数据、Work Area 与导出任务，并刷新预览
...
- **M5.2/M10 Remotion Code Agent**：AI 读写、注册、列出、注销用户项目内自定义 Remotion 组件源码，并支持混合撤销
...
| 数据分析 | Python 3.11 + FastAPI（开发可选；**安装版 bundled**） |
...
| 文档 | 说明 |
|------|------|
| [LLM-Agent设计.md](LLM-Agent设计.md) | LangChain Agent、M10 工具集、Remotion Code 与混合撤销 |
...
- **Python** 3.11+（`pnpm dev:all`、打安装包 `build:python` 时需要）
- **内存** 8GB+（推荐 16GB）
...
| `cd apps/electron && pnpm generate:preset-thumbnails` | 本地生成预设动图缩略图（Chrome + ffmpeg，WebP 不入库） |
...
```bash
# 需本地已有预设缩略图 WebP（generate:preset-thumbnails）
pnpm build:win
# → apps/electron/release/EasyMotion-Setup-0.1.2.exe
```

详细指南：[开发环境搭建指南.md](开发环境搭建指南.md) | [构建与部署.md](构建与部署.md) | 仓库根 [README.md](../../README.md)
...
│   │   ├── src/main/          # IPC、Agent、Generator、export-service、python-service
│   │   ├── src/renderer/src/  # React UI + Zustand stores
│   │   ├── electron-builder.yml
│   │   ├── scripts/           # 打包、缩略图 sync、F5 prepare
│   │   └── resources/         # templates、presets、python bundle（构建产物）
│   └── python/                # FastAPI（可选）
...
| 类别 | 变更 |
|------|------|
| **M10 Agent 增强** | 新增结构化读取、clip 移动、timeline settings、素材/数据工具、批量编辑、场景模板、参考图 layout 落盘、Work Area/导出工具 |
| **Remotion 生命周期** | 支持列出/注销自定义组件，删除前检查 timeline usages、锁定轨道与 `E2010` 冲突 |
| **混合撤销** | `agent-undo-snapshot.json` 扩展为 timeline + Remotion 文件 undo bundle，撤销前校验文件 hash 防止覆盖用户后续修改 |

## 历史变更（2026-06-30）

| 类别 | 变更 |
|------|------|
| **M9 收尾** | Windows 预发行 v0.1.2；M9 核心验收完成；E2E/签名/CI 延后 |
| **时间线** | AE 速度曲线、PR 标尺、关键帧与变换属性 UX |
| **打包** | `process-utils` 进包；安装版预览修复 |

## 历史变更（2026-06-27）

| 类别 | 变更 |
|------|------|
| **M9 打包** | `electron-builder` + `pnpm build:win`；Python venv bundled；`@langchain/core` 显式依赖 |
| **预设缩略图** | 安装版 `file://` 相对路径；`sync-preset-thumbnails.cjs` 构建前同步 WebP |
| **F5 调试** | `ensure-electron-binary.cjs`；根 `pnpm.onlyBuiltDependencies: electron` |
| **Remotion 预览** | 修复 `layers/*` 对 `apply-keyframes` 的错误相对路径；打开预览时自动修补旧项目 |
| **滚动条** | 属性栏、关键帧轨道与预设库统一为 `scrollbar-theme` |
| **素材库** | 移除右侧「素材」Tab；收藏 / 最近使用并入左侧 `AssetsPanel` |
| **右侧面板** | Tab 现为「属性 / 模板」 |
