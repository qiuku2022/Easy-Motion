...
Windows x64 安装包 **v0.1.1** 预发行版（未签名，SmartScreen 可能提示）：

- 本地构建产物：`apps/electron/release/EasyMotion-Setup-0.1.1.exe`
- 上一版 **v0.1.0-pre.1** 百度网盘：[EasyMotion-Setup-0.1.0.exe](https://pan.baidu.com/s/1IszD8X-GDhq-hjcH9cN0Lg?pwd=ncwr) · 提取码 `ncwr`

安装版已内置 Python，无需单独安装。首次使用 AI 请在应用内 **AI 助手 → 设置** 填入 API Key。问题反馈见 [Issues](https://github.com/qiuku2022/Easy-Motion/issues)。
...
| 产物 | 路径 |
|------|------|
| 安装程序 | `apps/electron/release/EasyMotion-Setup-0.1.1.exe` |
...
- **M9 打包**：`electron-builder` Windows NSIS；Python venv + 依赖 bundled；`pnpm build:win`
- 安装版预设缩略图路径修复（`file://` 相对路径）；构建前自动 sync 本地 WebP
- 主进程显式依赖 `@langchain/core`（修复安装版 Agent 启动报错）
- F5 增加 Electron 二进制自检；`pnpm.onlyBuiltDependencies` 预批准 electron
